...
(function(){
  ...
  window.renderMenu = (sidebarEl) => {
    ...
    if(loginLink){
      loginLink.addEventListener('click', (e) => { e.preventDefault(); window.location = 'login.html'; });
    }
  };
  ...
  function initInventario(){
    // protect
    if(getRole() !== 'admin'){ alert('Acceso restringido. Debes ingresar como administrador.'); window.location='login.html'; return; }
  ...
  // VENTAS (admin)
  function initVentas(){
    if(getRole() !== 'admin'){ alert('Acceso restringido'); window.location='login.html'; return; }
